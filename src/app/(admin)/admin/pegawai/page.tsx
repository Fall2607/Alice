...
export default function EmployeeManagementPage() {
  ...
  const [selectedLevel, setSelectedLevel] = useState<Option | null>(
    null
  );
  ...
  const confirmDelete = async () => {
    ...
    console.log("Menghapus:", selectedEmployee.nip);
    setEmployeeList(employeeList.filter(e => e.nip !== selectedEmployee.nip));
    handleCloseModals();
  };
  ...
  const getLevelBadgeClass = (levelName: string) => {
    switch (levelName) {
      case 'Direktur': return 'bg-purple-100 text-purple-800';
      case 'Wakil Direktur': return 'bg-indigo-100 text-indigo-800';
      case 'Supervisor': return 'bg-blue-100 text-blue-800';
      case 'Koordinator': return 'bg-teal-100 text-teal-800';
      case 'Staff': return 'bg-slate-100 text-slate-800';
  ...
  const getDepartmentBadgeClass = (departmentName: string) => {
    ...
    if (departmentName.toLowerCase().includes('igd') || departmentName.toLowerCase().includes('icu')) {
      return 'bg-red-100 text-red-800';
    }
    if (departmentName.toLowerCase().includes('rawat')) {
      return 'bg-sky-100 text-sky-800';
    }
    if (departmentName.toLowerCase().includes('farmasi') || departmentName.toLowerCase().includes('laboratorium') || departmentName.toLowerCase().includes('radiologi')) {
  ...
  return (
    <div className="p-8">
            ...
            <tbody>
              {isLoading ? (
                ...
                currentEmployees.map((employee) => (
                  <tr key={employee.nip} className="bg-white border-b border-slate-300 hover:bg-slate-50">
                    <th scope="row" className="px-6 py-4 font-medium text-slate-900">
                      <div className="flex items-center gap-2">
                        <span>{employee.nama_lengkap}</span>
                        {employee.sip && isSipExpired(employee.masa_berlaku_sip) && (
                          <div className="relative group">
                            <AlertTriangle size={16} className="text-red-500 cursor-pointer" />
                      ...
                      <p className="font-normal text-slate-500 text-xs">NIP: {employee.nip}</p>
                    </th>
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 text-xs font-semibold rounded-full ${getLevelBadgeClass(employee.nama_level)}`}>
                        {employee.nama_level || "-"}
                      </span>
                      <p className="text-slate-500 text-xs mt-1">
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${getDepartmentBadgeClass(employee.nama_departemen)}`}>
                          {employee.nama_departemen || "-"}
                        </span>
                      </p>
                    </td>
                    <td className="px-6 py-4"><p className="font-medium">{employee.handphone || "-"}</p><p className="text-xs text-slate-500">{employee.email || "-"}</p></td>
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 text-xs rounded-full ${employee.status_kepegawaian === "Karyawan Tetap" || employee.status_kepegawaian === "Dokter Tetap" || employee.status_kepegawaian === 'Aktif' ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>
                        {employee.status_kepegawaian}
                      </span>
                    </td>
                    <td className="px-6 py-4 flex items-center justify-center gap-4">
                      <button onClick={() => handleOpenViewModal(employee)} className="text-green-600 hover:text-green-800" title="Lihat Detail"><Eye size={18} /></button>
                      <Link href={`/admin/pegawai/edit/${employee.nip}`} className="text-blue-600 hover:text-blue-800" title="Edit"><Edit size={18} /></Link>
                ...
                <tr><td colSpan={5} className="text-center p-8 text-slate-500">Tidak ada data pegawai yang cocok.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <Modal isOpen={isViewModalOpen} onClose={handleCloseModals} title={`Detail Pegawai: ${selectedEmployee?.nama_lengkap}`} size="3xl">
        {selectedEmployee && (
          <div className="space-y-6">
            <section>
              <h3 className="font-semibold text-primary-dark border-b pb-2 mb-2">Data Diri & Kontak</h3>
              <dl className="divide-y divide-slate-100">
                <DetailRow label="NIP" value={selectedEmployee.nip} />
                <DetailRow label="Nama Lengkap" value={selectedEmployee.nama_lengkap} />
                <DetailRow label="NIK" value={selectedEmployee.nik} />
                <DetailRow label="Tanggal Lahir" value={selectedEmployee.tanggal_lahir ? new Date(selectedEmployee.tanggal_lahir).toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' }) : '-'} />
                <DetailRow label="Jenis Kelamin" value={selectedEmployee.jenis_kelamin} />
                <DetailRow label="No. Handphone" value={selectedEmployee.handphone} />
                <DetailRow label="Email" value={selectedEmployee.email} />
                <DetailRow label="Alamat" value={selectedEmployee.alamat} />
              </dl>
            </section>
            <section>
              <h3 className="font-semibold text-primary-dark border-b pb-2 mb-2">Informasi Kepegawaian & Profesional</h3>
              <dl className="divide-y divide-slate-100">
                <DetailRow label="Tanggal Masuk" value={selectedEmployee.tanggal_masuk ? new Date(selectedEmployee.tanggal_masuk).toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' }) : '-'} />
                <DetailRow label="Status Kepegawaian" value={selectedEmployee.status_kepegawaian} />
                <DetailRow label="Departemen" value={selectedEmployee.nama_departemen} />
                <DetailRow label="Level Jabatan" value={selectedEmployee.nama_level} />
                <DetailRow label="Profesi" value={selectedEmployee.profesi} />
                <DetailRow label="No. SIP" value={selectedEmployee.sip} />
                <DetailRow label="Masa Berlaku SIP" value={
                  <div className="flex items-center gap-2">
                    <span>{selectedEmployee.masa_berlaku_sip ? new Date(selectedEmployee.masa_berlaku_sip).toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' }) : '-'}</span>
                    {selectedEmployee.sip && isSipExpired(selectedEmployee.masa_berlaku_sip) && (
