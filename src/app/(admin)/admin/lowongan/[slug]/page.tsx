...
export default function DetailLowonganPage() {
    ...
    const [scoreModal, setScoreModal] = useState<{
        isOpen: boolean; applicantName: string; details: any[]; isLoading: boolean;
    }>({ isOpen: false, applicantName: '', details: [], isLoading: false });

    const baseUrl = process.env.NEXT_PUBLIC_API_BASE_URL || "";

    const handleViewScores = async (applicationStatusId: string, applicantName: string) => {
        setScoreModal({ isOpen: true, applicantName, details: [], isLoading: true });
        try {
            const res = await fetch(`/api/job-assessments/scores/${applicationStatusId}`);
            if (res.ok) {
            ...
            setDialog({ isOpen: true, type: 'alert', title: 'Error', message: 'Gagal memuat rincian skor.' });
        }
    };
    ...
    return (
        <div className="p-8 font-sans max-w-7xl mx-auto min-h-screen">
                                        ...
                                        applicants.map((applicant) => (
                                            <tr key={applicant.id} className={`hover:bg-slate-50 transition-all group ${selectedCandidates.includes(applicant.id) ? 'bg-blue-50/30' : ''}`}>
                                                ...
                                                <td className="px-8 py-6 text-center w-[150px]">
                                                    {applicant.suitability_match !== null && applicant.suitability_match !== undefined ? (
                                                        <div
                                                            className="flex flex-col items-center gap-1.5 cursor-pointer hover:bg-slate-100 p-2 rounded-lg transition-all"
                                                            onClick={() => applicant.application_status_id && handleViewScores(applicant.application_status_id, applicant.nama)}
                                                        ...
                                                        >
                                                            <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
                                                                <div
                                                                    ...
                                                                    style={{ width: `${Math.round(Number(applicant.suitability_match))}%` }}
                                                                ></div>
                                                            </div>
                                                            <span className={`text-xs font-black ${Number(applicant.suitability_match) >= 80 ? 'text-emerald-600' : Number(applicant.suitability_match) >= 50 ? 'text-amber-600' : 'text-red-600'}`}>
                                                                {Math.round(Number(applicant.suitability_match))}%
                                                            </span>
                                                        </div>
                                                    ) : (
            ...
            {/* SCORE DETAILS MODAL */}
            <Modal isOpen={scoreModal.isOpen} onClose={() => setScoreModal(prev => ({ ...prev, isOpen: false }))} title={`Detail Skor Assessment - ${scoreModal.applicantName}`} size="xl">
                <div className="p-2 overflow-y-auto max-h-[60vh]">
                    {scoreModal.isLoading ? (
                        <div className="flex justify-center items-center py-12"><Loader2 size={32} className="animate-spin text-primary" /></div>
                    ) : scoreModal.details.length === 0 ? (
                        <p className="text-center text-slate-500 font-medium bg-slate-50 p-6 rounded-lg">Tidak ada detail skor untuk kandidat ini.</p>
                    ) : (
                        <div className="space-y-3">
                            {scoreModal.details.map((item, idx) => (
                                <div key={idx} className="p-4 rounded-xl border border-slate-100 bg-slate-50 flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
                                    <div className="flex-1">
                                        <p className="text-sm font-bold text-slate-700 mb-1.5">{item.question}</p>
                                        <div className="text-xs text-slate-500 flex items-center gap-2">
                                            Jawaban: <span className="font-bold text-primary bg-primary/10 px-2 py-0.5 rounded border border-primary/20">{item.answer_value}</span>
                                        </div>
                                    </div>
                                    <div className="flex flex-col items-end shrink-0">
                                        <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1">Skor</div>
                                        <div className={`text-xl font-black ${item.fuzzy_score >= 80 ? 'text-emerald-500' : item.fuzzy_score >= 50 ? 'text-amber-500' : 'text-red-500'}`}>
                ...
                <div className="mt-6 flex justify-end pt-4 border-t border-slate-100">
                    <button onClick={() => setScoreModal(prev => ({ ...prev, isOpen: false }))} className="px-6 py-2.5 bg-slate-800 text-white font-bold rounded-lg hover:bg-slate-700 transition-all text-xs uppercase tracking-widest shadow-md">
                        Tutup Panel
                    </button>
                </div>
            </Modal>
