...
export default function JabatanManagementPage() {
  ...
  const [selectedJabatan, setSelectedJabatan] = useState<LevelJabatan | null>(
    null
  );
  ...
  const fetchJabatans = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`${baseUrl}/level-jabatan`);
      if (!response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = isEditing
      ? `${baseUrl}/level-jabatan/${selectedJabatan?.id}`
      : `${baseUrl}/level-jabatan`;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message ||
            `Gagal ${isEditing ? "memperbarui" : "menambah"} data.`
        );
      }
  ...
  return (
    <div className="p-8">
                ...
                jabatans.map((jabatan) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 flex items-center justify-center gap-4">
                      <button
                        onClick={() => handleOpenEditModal(jabatan)}
                      ...
                      <button
                        onClick={() => handleOpenDeleteModal(jabatan)}
      ...
      <Modal
        ...
        title={
          isAddModalOpen ? "Tambah Level Jabatan Baru" : "Edit Level Jabatan"
        }
      >
      ...
      >
        <div>
          <p>
            Apakah Anda yakin ingin menghapus level jabatan{" "}
            <strong>{selectedJabatan?.nama_level}</strong>?
          </p>
          <div className="mt-6 flex justify-end gap-4">
            <button
              onClick={handleCloseModals}
